...
image:flexpod-fips-image3.png["A diagram that shows encrypted write and decrypted read data flow through network, WAFL, RAID, and storage with a cryptographic module"]

[NOTE]
NVE uses a software cryptographic module which is FIPS 140-2 level 1 validated.

For more information about NVE, see the https://www.netapp.com/pdf.html?item=/media/17070-ds-3899.pdf[NVE Datasheet^].
...
image:flexpod-fips-image4.png["A diagram that shows encryption and decryption through Network, WAFL, RAID, and Storage cryptographic module layers"]

[NOTE]
NSE uses hardware encryption on each drive,  which is FIPS 140-2 level 2 validated.

For more information about NSE, see the https://www.netapp.com/pdf.html?item=/media/7563-ds-3213-en.pdf[NSE datasheet^].

== Key management

The FIPS 140-2 standard applies to the cryptographic module as defined by the boundary, as shown in the following figure.

image:flexpod-fips-image5.png["A diagram that shows the ONTAP cryptographic boundary with key manager, cryptomod, and cryptomod_fips components"]
