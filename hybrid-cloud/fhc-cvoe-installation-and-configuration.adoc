...
. Access the NetApp Console.
+
To access the Console and other cloud services, you need to sign up on https://cloudmanager.netapp.com/[NetApp Console^]. For setting up workspaces and users in the Console account, see https://docs.netapp.com/us-en/console-setup-admin/index.html[NetApp Console setup and administration^]. You need an account that has permission to deploy the Console agent in your cloud provider directly from the Console. To obtain the permissions you need, refer to https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions.html[Permissions summary for NetApp Console^].

. Deploy Console agent.
+
Before adding a Cloud Volume ONTAP system, you must deploy a Console agent. The Console prompts you if you try to create your first Cloud Volumes ONTAP system without a Console agent in place. To deploy a Console agent in AWS from the Console, see the https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-creating-connectors-aws.html[Console agent installation options in AWS^].
...
For deploying the on-premises FlexPod instance, see https://www.cisco.com/c/en/us/td/docs/unified_computing/ucs/UCS_CVDs/flexpod_xseries_vmware_7u2.html[this deployment guide^].
