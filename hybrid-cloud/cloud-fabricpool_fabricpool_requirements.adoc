...
In releases earlier than ONTAP 9.8, FabricPool is only supported on SSD local tiers. Support for HDD aggregates was added in ONTAP 9.8.
For entry-level AFF platforms that have a fixed, relatively small capacity such as the NetApp AFF C190, FabricPool can be highly effective for moving inactive data to the cloud tier.
...
[NOTE]
Please refer to the https://hwu.netapp.com/DetailViews/Home.aspx[NetApp Hardware Universe^] for the latest information on supported platforms with FabricPool.

* NetApp ONTAP Select
[NOTE]
NetApp recommends using all-SSD FabricPool local tiers.
...
image:cloud-fabricpool_image6.png["A diagram that shows two NetApp AFF A300 controllers connecting intercluster LIFs to AWS S3"]
...
image:cloud-fabricpool_image7.png["A screenshot of ONTAP CLI object store profiler results showing PUT and GET latency and throughput"]
...
FabricPool licenses (including additional capacity for existing licenses) can be purchased in 1TB increments.
