...
* A separate 1Gb Ethernet out-of-band management network is required for all components.
* NetApp recommends that you enable jumbo frame support throughout the environment, but it is not required.
* Standalone Cisco UCS Fabric Interconnect appliance ports are recommended only for iSCSI connections. When NAS connections are used with appliance ports, additional Nexus / compliant switches are required for certain failure scenarios.
* No additional equipment can be placed in line between the core FlexPod components.

Uplink connections:

* The ports on the NetApp storage controllers must be connected to the Cisco Nexus 9000 Series Switches to enable support for virtual port channels (vPCs).
* vPCs are required from the Cisco Nexus 9000 Series Switches to the NetApp storage controllers.
* vPCs are required from the Cisco Nexus 9000 Series Switches to the fabric interconnects.
...
* NetApp storage controller ports that are directly connected to the fabric interconnects (as in the case of X-Series Direct) can be grouped to enable a port channel. vPC is not supported for this configuration.
...
Refer to the link:https://docs.netapp.com/us-en/flexpod/fp-def/dc-tech-spec_topologies.html[FlexPod Datacenter topologies^] section for connectivity requirements specific to each topology.