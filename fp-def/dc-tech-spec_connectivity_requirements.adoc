...
* Standalone Cisco UCS Fabric Interconnect appliance ports are recommended only for iSCSI connections. When NAS connections are used with appliance ports, additional Cisco Nexus or equivalent ethernet switches are required for certain failure scenarios.
* No additional equipment can be placed in line between the core FlexPod components.

Uplink connections:

* The ports on the NetApp storage controllers must be connected to the Cisco Nexus 9000 Series Switches.
