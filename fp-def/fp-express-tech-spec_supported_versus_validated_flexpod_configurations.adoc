...
[.lead]
The FlexPod architecture is defined by the set of rules described in this document. The hardware components and software configurations must be supported by the https://ucshcltool.cloudapps.cisco.com/public[Cisco Hardware Compatibility List (HCL)^] and the http://mysupport.netapp.com/matrix[NetApp Interoperability Matrix Tool (IMT)^].
