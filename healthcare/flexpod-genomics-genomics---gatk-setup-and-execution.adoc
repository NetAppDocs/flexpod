...
image:flexpod-genomics-image8.png["A diagram that shows a genomics workflow from sample collection through sequencing, variant analysis, EHR integration, and predictive models"]
...
+
image:flexpod-genomics-image10.png["A table of VLAN IP reservations showing ranges, subnet masks, and management, VM, NFS, and iSCSI purposes"]

. Configure iSCSI-based boot LUNs on the ONTAP SVM.
+
image:flexpod-genomics-image9.png["A screenshot of ONTAP System Manager showing six ESXi boot LUNs in the Healthcare_SVM storage VM"]

. Map LUNs to iSCSI initiator groups.
+
image:flexpod-genomics-image11.png["A screenshot that shows ESXi_Boot_Lun_1 mapped to the GenomicsESXi_1 iSCSI initiator group"]
+
image:flexpod-genomics-image12.png["A screenshot of ONTAP System Manager showing ESXi boot LUN details mapped to the GenomicsESXi_2 initiator group"]

. Install vSphere 7.0 with iSCSI boot.
. Register ESXi hosts with the vCenter.
+
image:flexpod-genomics-image13.png["A screenshot of vSphere Client inventory showing the Genomics folder with ESXi hosts and virtual machines"]

. Provision an NFS datastore `infra_datastore_nfs` on the ONTAP storage.
+
image:flexpod-genomics-image14.png["A screenshot of ONTAP System Manager showing the selected Healthcare_Workload_NFS_Vol NFS volume online"]

. Add the datastore to the vCenter.
+
image:flexpod-genomics-image16.png["A screenshot of the vSphere Client showing the infra_datastore_nfs NFS 3 datastore summary"]

. Using vCenter, add an NFS datastore to the ESXi hosts.
+
image:flexpod-genomics-image15.png["A screenshot of vSphere Client showing infra_datastore_nfs hosts connected with normal status"]
...
+
image:flexpod-genomics-image17.png["A screenshot of a Linux terminal showing /dev/sdb mounted at /mnt/genomics with 2% usage"]
